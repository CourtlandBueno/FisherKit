...
let testImagePNGData = testImage.fk.pngRepresentation()!
let testImageJEPGData = testImage.fk.jpegRepresentation(compressionQuality: 1.0)!
...
func clearCaches<Item: FisherKitItemType>(_ caches: [FisherKitManager<Item>.Cache]) {
    for c in caches {
        c.clearMemoryCache()
...
extension Bundle {
    static let test: Bundle = Bundle(for: FisherKitManagerTests.self)
}
